...
void CIriFitnessFunction::SimulationStep(unsigned int n_simulation_step, double f_time, double f_step_interval)
{
	...
	/* F1 */
	// double fitness = (lightS7*lightS0*(1-maxProxSensorEval)*(1-maxContactSensorEval)*maxLightSensorEval);

	/* F2 */
	// double fitness = (lightS7+lightS0+(1-maxProxSensorEval)+(1-maxContactSensorEval)+maxLightSensorEval)*(leftSpeed*rightSpeed)/5;

	/* F3 */
	// double fitness = ((1-maxProxSensorEval)*(1-maxContactSensorEval)+maxLightSensorEval+lightS7+lightS0)/4;

	/* F4 */
	// double fitness = ((1-maxProxSensorEval)*(1-maxContactSensorEval)*(maxLightSensorEval+lightS7+lightS0))/3;

	/* F5 */
	// double fitness = ((1-maxProxSensorEval)*(maxLightSensorEval+lightS7+lightS0))/3;

	/* F6 */
	// double fitness = coef1*(goForward*(maxProxSensorEval)) + coef2*(battery[0]) + coef4*(maxLightSensorEval) + coef3*(blueBattery[0]) 
	// 	+ (1 - coef1 - coef3 - coef2 - coef4)*(maxBlueLightSensorEval)
	// ; 

	/* F7 */
	double fitness = leftSpeed*rightSpeed*((1 - maxProxSensorEval))*(battery[0])
		*((maxLightSensorEval+lightS7+lightS0)3)*(blueBattery[0])
		*((maxBlueLightSensorEval+blueLightS7+blueLightS0)/3)
	;
